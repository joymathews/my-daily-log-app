...
module.exports = function(app, deps) {
  ...
  app.get('/view-events', authenticateJWT, apiLimiter, async (req, res) => {
  ...
  app.get('/view-events-by-date', authenticateJWT, apiLimiter, async (req, res) => {
    const userSub = req.user.sub;
  ...
  app.get('/event-dates', authenticateJWT, apiLimiter, async (req, res) => {
    const userSub = req.user.sub;
    try {
      const params = {
        TableName: DYNAMODB_TABLE_NAME,
